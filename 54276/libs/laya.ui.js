(function (exports, Laya) {
    ...
    class Slider extends UIComponent {
        ...
        set showProgress(value) {
            ...
            if (value) {
                if (this._skin && !Laya.SerializeUtil.isDeserializing)
            ...
            if (url) {
                return Laya.AssetDb.inst.resolveURL(url).then(url => {
                    ...
                    if (this._showProgress)
                        tasks.push(this._progress._setSkin(Laya.Utils.replaceFileExtension(url, "$progress.png", true)));
                    else
                        this._progress.skin = null;
    ...
    class OpenDataContextView extends UIComponent {
        constructor() {
            ...
            let tex = new Laya.Texture(new Laya.Texture2D(this._width, this._height, Laya.TextureFormat.R8G8B8A8, false, false, true));
        ...
        _onLoop() {
            ...
            if (tex.width != canvas.width || tex.height != canvas.height) {
                tex.bitmap.destroy();
                tex.bitmap = new Laya.Texture2D(canvas.width, canvas.height, Laya.TextureFormat.R8G8B8A8, false, false, true, true);
                tex.bitmap.lock = true;
            }
    ...
    class StatUI {
        ...
        showToggle(x, y, views) {
            x = x || 0;
            y = y || 0;
            views = views || Laya.Stat.AllToggle;
            this._toggleView = views;
            this._showToggle = true;
            if (!this._toggleSprite)
                this.createToggleUI();
            Laya.Laya.stage.addChild(this._toggleSprite);
        ...
        update() {
            ...
            if (this._showToggle) {
                for (let i = 0; i < this._toggleView.length; i++) {
                    ...
                    Laya.Stat[one.value] = this._checkBoxArray[i].selected;
                }
            }
        }
        createUI() {
            let sp = this._sp = new Laya.Sprite();
            sp.scale(Math.max(Laya.Laya.stage.clientScaleX, 1), Math.max(Laya.Laya.stage.clientScaleY, 1));
            Laya.Laya.stage.on(Laya.Event.RESIZE, this, () => {
                this._sp.scale(Math.max(Laya.Laya.stage.clientScaleX, 1), Math.max(Laya.Laya.stage.clientScaleY, 1));
            });
            let leftText = new Laya.Text();
            leftText.pos(5, 5);
        ...
        createToggleUI() {
            if (!checkBoxTex) {
                ...
                pixels[7] = 128;
                pixels[8] = 0;
                checkBoxTex = new Laya.Texture2D(1, 3, Laya.TextureFormat.R8G8B8, false, false);
                checkBoxTex.setPixelsData(pixels, false, false);
                checkBoxTex.lock = true;
                checkBoxTex.name = "StatUICheckBox";
                Laya.Loader.cacheRes(checkBoxTex.name, checkBoxTex, Laya.Loader.TEXTURE2D);
            }
            let sp = this._toggleSprite = new Laya.Sprite();
            sp.zOrder = 1000000;
            let leftText = new Laya.Text();
            leftText.pos(5, 5);
            leftText.color = "#ffffff";
            ...
            for (let i = 0; i < this._toggleView.length; i++) {
                ...
                cb.selected = Laya.Stat[one.value];
                cb.scale(12, 12);
                cb.pos(0, i * h + 2);
                cb.size(12, 12);
                toggles.addChild(cb);
