let CropPuzzle = new Phaser.Class({
 
	Extends: Phaser.GameObjects.Container,   

	initialize:

	function CropPuzzle(scene)
	{
		this.scene = game_data['scene'];
		Phaser.GameObjects.Container.call(this, this.scene, 0, 0);        
		this.emitter = new Phaser.Events.EventEmitter();
        this.tries_amount = game_data['tries_allowed']['crop'];
	},
    ...
    create_crop_items() {
        ...
        let start_x = dx;
        this.crop_items = this.items.map(id => {
            let item = new Phaser.GameObjects.Image(this.scene, dx, dy, id).setScale(0.7).setOrigin(0.5);
            item.id = id;
            if (this.crop_type === 'horizontal') {
                item.setCrop(0, 0, item.width / 2, item.height);
            }
            else if (this.crop_type === 'vertical') {
                item.setCrop(0, 0, item.width, item.height / 2);
            }
            this.add(item);
            dx+=150;
            if (dx > limit_x) {
    ...
    create_target_items() {
        ...
        this.target_items = this.items.map(id => {
            ...
            item.bg = rect;
            if (this.crop_type === 'horizontal') {
                item.setCrop(item.width / 2, 0, item.width / 2, item.height);
            }
            else if (this.crop_type === 'vertical') {
                item.setCrop(0, item.height / 2, item.width, item.height / 2);
            }
            ...
            item.on('dragend', function(pointer, dragX, dragY, dropped){ 
                let no_overlap = true;
                for (let i = 0; i < this.crop_items.length; i++) {
                    let item2 = this.crop_items[i];
    ...
    update_level(params) {
        ...
        let { items, crop_type } = this.level_info;
        ...
        else if (!['vertical', 'horizontal'].includes(crop_type)) throw new Error('Level is broken, crop_type is incorrect');
        this.items = items;
