!(function () {
  class WebAudioEngine {
    ...
    init() {
      return new Promise((resolve, reject) => {
        try {
          ...
          this.musicAudio.getContext().onstatechange =
            this.onMusicStatechange.bind(this);
          this.soundAudio.getContext().onstatechange =
            this.onSoundStatechange.bind(this);
          this.beEnabled = true;
          this.musicVolume = 60;
    ...
    onVisibilitychange() {
      ...
      if (document.visibilityState == "hidden") {
        ...
        Laya.physicsTimer && Laya.physicsTimer.pause();
    ...
    tryToResumeAudioContext() {
      ...
      if (!this.musicAudio.isSuspend() || !this.soundAudio.isSuspend()) {
        window.document.removeEventListener(
          "mousedown",
          this.tryToResumeAudioContext.bind(this),
          true,
        );
        window.document.removeEventListener(
          "touchstart",
    ...
    onMusicStatechange() {
      if (
        ...
        this.tryToResumeIntervalId = setInterval(
          this.tryToResumeAudioContext.bind(this),
          0.2e3,
        );
      }
  ...
  class WebAudioContext {
    ...
    play(url, loop = false, singleton = false) {
      if (this._audioInstances.has(url)) {
        const sound = this._audioInstances.get(url);
        const instance = sound.instance;
        if (singleton && !instance.ended) return;
        this.stop(url);
        if (sound.buffer) {
          try {
  ...
  class platform {
    ...
    showReward1(success, failure, com) {
    ...
    showReward(success, failure) {
      window.showVideoAd([
        ...
        () => {
          failure && failure();
          Laya.Dialog.open(
            "view/ToastDialog.scene",
            false,
            "跳过播放视频无法获得奖励",
          );
          //this.prompt("跳过播放视频无法获得奖励");
        },
        () => {
          Laya.Dialog.open("view/ToastDialog.scene", false, "暂时无法播放视频");
          failure && failure();
          //this.prompt("暂时无法播放视频");
        },
      ]);
    }
    initList(appList) {}
    prompt(msg, duration) {
      if (!this.prompt_) {
        ...
        this.prompt_.style.cssText =
          "overflow: hidden;word-break: break-all;word-wrap: break-word;font-family:siyuan;padding:10px 10px 10px 10px;min-height:20px;color: rgb(255, 255, 255);line-height: 20px;text-align:center;border-radius: 4px;position: fixed;top: 40%;left: 50%;transform: translate(-50%, -50%);z-index: 999999;background: rgba(0, 0, 0,.7);font-size: 14px;";
    ...
    prompt1(msg, duration) {
      ...
      setTimeout(
        function () {
          var d = 0.5;
          this.prompt_q.style.webkitTransition =
            "-webkit-transform " + d + "s ease-in, opacity " + d + "s ease-in";
      ...
      class noVideoScript extends Laya.Script {
        ...
        onEnable() {
          ...
          if (this.owner.width > this.owner.height) {
            this.spr_tip.scale(
              this.owner.height / 1920,
              this.owner.height / 1920,
            );
          } else {
            this.spr_tip.scale(
              this.owner.width / 1080,
              this.owner.width / 1080,
            );
          }
    ...
    scrollList() {
      ...
      if (
        YYGGames.getAdPlatformType() == AdPlatformType.en_GAMEDISTRIBUTION ||
        YYGGames.getAdPlatformType() == AdPlatformType.en_XIAOMI
      ) {
      ...
      if (!this._scrollList) {
        ...
        class scrollListScript extends Laya.Script {
          ...
          onEnable() {
            ...
            !this.owner.renderHandler &&
              (this.owner.renderHandler = new Laya.Handler(
                this,
                this.itemRenderFun,
              ));
          ...
          itemRenderFun(e, index) {
            ...
            if (this.sizeTran[2]) {
              ...
              let scaleNum =
                200 / this.sizeTran[0] < 150 / this.sizeTran[1]
                  ? 200 / this.sizeTran[0]
                  : 150 / this.sizeTran[1];
        ...
        this._scrollList.setSize = this._scrollList
          .getComponent(scrollListScript)
    ...
    box_adTwo() {
      ...
      if (!this._box_adTwo) {
        ...
        class box_adTwoScript extends Laya.Script {
          ...
          onEnable() {
            !this.img_ad0 &&
              (this.img_ad0 = this.owner.getChildByName("img_ad0"));
            !this.img_ad1 &&
              (this.img_ad1 = this.owner.getChildByName("img_ad1"));
            !this.img_ad0Ad &&
              (this.img_ad0Ad = this.img_ad0.getChildByName("img_adImg"));
            !this.img_ad1Ad &&
              (this.img_ad1Ad = this.img_ad1.getChildByName("img_adImg"));
          ...
          setSize(width, height, isTranImg = false) {
            ...
            if (this.img_ad0) {
              ...
              if (isTranImg) {
                this.img_ad0AdMask.width =
                  this.img_ad1AdMask.width =
                  this.img_ad0Ad.width =
                  this.img_ad1Ad.width =
                    width;
                this.img_ad0AdMask.height =
                  this.img_ad1AdMask.height =
                  this.img_ad0Ad.height =
                  this.img_ad1Ad.height =
                    height;
              } else {
                let scaleNum =
                  200 / width < 150 / height ? 200 / width : 150 / height;
              ...
              this.img_ad0AdMask.x = this.img_ad1AdMask.x =
                this.img_ad0Ad.width / 2;
              this.img_ad0AdMask.y = this.img_ad1AdMask.y =
                this.img_ad0Ad.height / 2;
              this.setSpaceX(this.spaceNum);
            }
          }
        }
        ...
        this._box_adTwo.setSpaceX = this._box_adTwo
          .getComponent(box_adTwoScript)
          .setSpaceX.bind(this._box_adTwo.getComponent(box_adTwoScript));
        this._box_adTwo.setSize = this._box_adTwo
          .getComponent(box_adTwoScript)
  ...
  class OtherUI {
    ...
    renderADHandler(box, index) {
      ...
      let maxNum = 200;
      if (
        this._scrollList.sizeArr[0] / 200 >
        this._scrollList.sizeArr[1] / 150
      ) {
      ...
      let realWidth = 200;
      let realHeight =
        (200 * this._scrollList.sizeArr[1]) / this._scrollList.sizeArr[0];
